...
class BaseDrop:
    def __init__(
        ...
        self._twitch: TwitchClient = campaign._twitch
        self.id: str = data["id"]
        ...
        bugged_time = datetime(1, 1, 1, 0, 0, tzinfo=timezone.utc)
        if "self" in data:
...
class DropsCampaign:
    def __init__(
        self, twitch: TwitchClient, data: JsonType, claimed_benefits: dict[str, datetime]
    ):
