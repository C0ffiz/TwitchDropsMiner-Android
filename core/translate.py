"""Translation / i18n support for TwitchDropsMiner Android.

Ported from TwitchDropsMiner/translate.py.  Android-specific changes are
marked with ``# Android-specific`` comments and summarised at the bottom of
this module.
"""
...
from core.utils import json_load, json_save                      # Android-specific: core.utils
from core.constants import DEFAULT_LANG

if TYPE_CHECKING:
    from typing_extensions import NotRequired

# Android-specific: LANG_PATH is computed relative to this file so it works
# both on-device and in desktop dev sessions without App.get_running_app().
LANG_PATH: Path = Path(__file__).parent.parent / "lang"          # Android-specific


# ---------------------------------------------------------------------------
# TypedDict hierarchy — ported exactly from upstream
# ---------------------------------------------------------------------------
...
class GUISettingsNotifications(TypedDict):          # Android-specific: TypedDict added for
    name: str                                        # the "notifications" key present in
    url: str                                         # default_translation but absent upstream
    test: str
...
class GUISettings(TypedDict):
    ...
    notifications: GUISettingsNotifications          # Android-specific: added to match data
...
# ---------------------------------------------------------------------------
# Built-in English translation (fallback / default)
# ---------------------------------------------------------------------------
...
class Translator:
    def __init__(self) -> None:
        ...
        # Android-specific: IS_PACKAGED branch removed — we never write lang files on Android.
        # The upstream dev-mode json_save call is intentionally omitted here.
    ...
    def set_language(self, language: str) -> None:
        if language not in self._langs:
            ...
            # Android-specific: sync load acceptable at startup; graceful fallback to English
            # if the lang file is missing or corrupt on the device.
            try:
                self._translation = json_load(
                    LANG_PATH / f"{language}.json", default_translation
                )
            except Exception:
                self._translation = default_translation.copy()
    ...
    def available_languages(self) -> list[str]:
        ...
        Android-specific: convenience method for the settings screen language picker.
        Rescans the lang folder so newly-added files are detected without restart.
        """
        langs: list[str] = []
        for filepath in LANG_PATH.glob("*.json"):
            langs.append(filepath.stem)
        langs.sort()
    ...
    def __call__(self, *path: str) -> str:
        ...
        try:
            ...
            raise MinerException(
                ...
                f"'{' -> '.join(path)}' translation key"
            )
        return v


# Module-level singleton — identical role to upstream's ``_ = Translator()``
_ = Translator()


# ---------------------------------------------------------------------------
# Summary of deliberate changes from upstream translate.py
# ---------------------------------------------------------------------------
# 1. Imports changed to ``core.*`` namespace (core.exceptions, core.utils,
#    core.constants) instead of flat module paths.
# 2. LANG_PATH: computed as ``Path(__file__).parent.parent / "lang"`` rather
#    than imported from constants (it was a desktop runtime path there).
# 3. IS_PACKAGED branch removed: on Android we never write lang files back to
#    disk; the dev-mode ``json_save(English.json)`` call is omitted entirely.
# 4. GUISettingsNotifications TypedDict added: the "notifications" key exists
#    in default_translation but had no corresponding TypedDict upstream.
# 5. GUISettings.notifications field added to match the new TypedDict above.
# 6. set_language() wraps json_load in try/except for Android graceful fallback
#    if a lang file is missing or corrupt on the device.
# 7. available_languages() added as an Android-specific helper for the
