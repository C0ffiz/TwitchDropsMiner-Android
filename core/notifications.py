...
try:
    ...
    _apprise_mod = None  # type: ignore[assignment]
    _APPRISE_AVAILABLE = False  # Android-specific: graceful fallback if apprise not in build
...
try:
    ...
    _PLYER_AVAILABLE = False  # Android-specific: graceful fallback

from core.utils import RateLimiter, notification_urls
...
if TYPE_CHECKING:
    from core.inventory import TimedDrop


# ---------------------------------------------------------------------------
# Discord embed payload helper (unchanged from upstream)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Abstract base — created for Android port (no base existed in upstream)
# ---------------------------------------------------------------------------

class NotificationBase:  # Android-specific
    """Abstract base for notification backends."""

    def send(self, title: str, message: str, icon: str = "") -> None:
        raise NotImplementedError


# ---------------------------------------------------------------------------
# Android local-device notification backend
# ---------------------------------------------------------------------------

class AndroidNotification(NotificationBase):  # Android-specific
    """Android notification backend using plyer."""

    def __init__(self) -> None:
        self.enabled: bool = True  # Android-specific

    def send(self, title: str, message: str, icon: str = "") -> None:
        if not self.enabled:  # Android-specific
            return
        # Android-specific: TODO — differentiate foreground vs background
        # notifications using Kivy App.get_running_app().state
        ...
        try:
            plyer_notification.notify(
                title=title,
                message=message,
                app_name="TwitchDropsMiner",
                timeout=5,
            )
        except Exception:
            pass  # Android-specific: silently skip if notification fails


def get_notifier() -> AndroidNotification:  # Android-specific
    """Factory — always returns AndroidNotification on Android."""
    return AndroidNotification()


# ---------------------------------------------------------------------------
# Webhook / service notifier (Discord, Telegram, etc. via apprise + aiohttp)
# ---------------------------------------------------------------------------
...
class AppriseNotifier:
    def __init__(self, get_session: Callable[[], abc.Awaitable[aiohttp.ClientSession]]):
        ...
        self._urls: list[str] = []
        self._apprise: Any = _apprise_mod.Apprise() if _APPRISE_AVAILABLE else None
    ...
    def reload(self, urls: str | abc.Iterable[str]) -> None:
        ...
        if _APPRISE_AVAILABLE:
            self._apprise = _apprise_mod.Apprise()
            if self._urls:
    ...
    async def _notify(self, *, embed: DiscordEmbed, title: str, body: str) -> None:
        try:
            ...
            if other_urls and _APPRISE_AVAILABLE:
                apprise_client = _apprise_mod.Apprise()
...
# ---------------------------------------------------------------------------
# URL parsing helper (unchanged from upstream)
# ---------------------------------------------------------------------------
